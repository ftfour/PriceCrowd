<template>
  <section class="space-y-6">
    ...
    <div class="rounded-lg border bg-white divide-y">
      <div v-for="p in filtered" :key="p._id" class="p-4 flex items-center gap-4">
        <div class="h-16 w-16 sm:h-20 sm:w-20 rounded-xl bg-slate-50 border flex items-center justify-center overflow-hidden">
          <img :src="toAbs(p.image_url)" alt="product" class="max-h-14 sm:max-h-16 object-contain" />
        </div>
        <div class="flex-1 min-w-0">
          ...
          <div class="mt-2 flex flex-wrap gap-2">
            <span v-for="cid in p.category_ids" :key="cid" class="text-xs bg-slate-100 border border-slate-200 rounded px-2 py-1">{{ categoryName(cid) }}</span>
          </div>
        </div>
      </div>
...
const filtered = computed(() => {
  ...
  return products.value.filter(p => p.title.toLowerCase().includes(q) || p.desc.toLowerCase().includes(q));
});
...
async function fetchProducts() {
  ...
  if (resCats.ok) {
    ...
    categories.value = list.map((c:any)=> ({...c, _id_str: (typeof c._id === 'string' ? c._id : c._id?.$oid)}));
  }
}
