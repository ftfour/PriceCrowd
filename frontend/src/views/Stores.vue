  ...
  <section class="space-y-8">
    ...
    <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
      <article
        v-for="s in filtered"
        :key="s._id"
        class="rounded-lg border bg-white p-0 overflow-hidden flex flex-col"
      >
        <div class="h-40 bg-slate-50 flex items-center justify-center border-b">
          <img
            :src="toAbs(s.image_url)"
            alt="store"
        ...
        <div class="p-4 flex flex-col gap-3">
          <header class="flex items-start justify-between gap-2">
            ...
            <div class="flex gap-2" v-if="isAdmin">
              ...
              <button
                @click="removeStore(s)"
                class="rounded-md bg-red-600 text-white px-2 py-1 text-xs"
              >
                Удалить
              </button>
            </div>
