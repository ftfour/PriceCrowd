  ...
  <section class="space-y-6">
    <div class="flex items-end justify-between gap-4">
      ...
      <form @submit.prevent="create" class="flex items-center gap-2">
        ...
        <button class="h-9 px-3 rounded bg-blue-600 text-white text-sm">Создать</button>
      </form>
    </div>
...
async function create(){
  ...
  if (!payload.username || payload.password.length<4) return;
  ...
  if (res.ok){ newUser.value = { username: '', password: '' }; await load(); }
}
...
async function save(u: User){
  ...
  if (res.ok || res.status===204){ cancel(); await load(); }
}
