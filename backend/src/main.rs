...
async fn main() -> anyhow::Result<()> {
    ...
    let app = Router::new()
        ...
        .route("/stores/:id/products/insights", get(list_store_product_insights))
...
async fn list_store_product_insights(State(state): State<AppState>, Path(id): Path<String>) -> impl IntoResponse {
    let Ok(store_oid) = ObjectId::from_str(&id) else { return StatusCode::BAD_REQUEST.into_response(); };
